...
abstract class Validations {
    ...
    static Validation<ErrorMessage, Money> validateAmount(String currency, String amount, Set<CurrencyUnit> expectedCurrencies) {
        ...
        return Try.of(() -> new BigDecimal(amount))
                ...
                .getOrElse(invalid(error("Amount {} {} is not valid positive decimal with expected currency {}", amount, currency, expectedCurrencies)));
    }
    ...
    static Validation<ErrorMessage, Payment.Type> validateType(String type) {
        ...
        return Try.of(() -> Payment.Type.valueOf(type))
                ...
                .getOrElse(() -> invalid(error("Payment type {} is invalid. Supported types {}.", type, Payment.Type.values())));
    }

    static Validation<ErrorMessage, Iban> validateIban(String owner, String iban) {
        if (iban == null) return invalid(error("{} IBAN is required", owner));
        return IBAN_PATTERN.matcher(iban).matches()
                ? valid(new Iban(iban))
                : invalid(error("{} IBAN does not match pattern: {}", owner, IBAN_PATTERN.pattern()));
    }

    static Validation<ErrorMessage, Bic> validateBic(String owner, String bic) {
        if (bic == null) return invalid(error("{} BIC is required", owner));
        return BIC_PATTERN.matcher(bic).matches()
                ? valid(new Bic(bic))
                : invalid(error("{} BIC does not match pattern: {}", owner, BIC_PATTERN.pattern()));
    }
}
